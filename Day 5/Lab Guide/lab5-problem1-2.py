...
def costFunction_regularized_logistic_regression(theta_param, lambda_param, X, y):
    ...
    cost = -1*(1/number_of_examples)*(np.log(predicted_y).T.dot(y) + \
                                      np.log(1-predicted_y).T.dot(1-y))+ \
...
def gradient_regularized_logistic_regression(theta_param, lambda_param, X, y):
    ...
    + (lambda_param / number_of_examples) * \
        np.r_[[[0]], theta_param[1:].reshape]
...
def predict(theta_param, X, threshold=0.5):
    predicted_y = # Put your code here
    return predicted_y.astype('int')
...
initial_theta = # Put your code here to initialize 0-vector
