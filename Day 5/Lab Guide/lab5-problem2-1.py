...
lambda_param = tf.constant(10000.0, name='lambda_constant')
...
cost_function = tf.multiply(tf.divide(1, 2), tf.reduce_mean(tf.pow(Y - hypothesis_function, 2)) +
                            (lambda_param * tf.reduce_sum(tf.math.pow(theta0,2))))
...
with tf.Session() as session:
    ...
    for i in range(30000):
        ...
        cost = session.run(cost_function, feed_dict={X: data.T[0], Y: data.T[1]})
