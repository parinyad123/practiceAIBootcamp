...
def costfunction(Theta1, Theta2, X_with_bias, y_one_hot, lambda_param):
    ...
    J = -1 * (1 / number_of_examples) \
        * np.sum((np.log(a3.T) * (y_one_hot) + np.log(1-a3).T * (1-y_one_hot))) \
...
'''
print('The cost (without regularization) is', costfunction(Theta1, Theta2, X_new, y_one_hot, 1))